...
def main():
    classes = ['BPSK', 'QPSK', '8PSK', 'QAM16', 'QAM64']#你自己重新分类！有几个标签就写几类，标签用one-hot编码，你查一下

    # X_train = np.load('train_set_swt_lv1.npy')

    # X_test = np.load('test_set_swt_lv1.npy')

    X_train = np.load('X_train.npy')#数据

    X_test = np.load('X_test.npy')

    Y_train = np.load('train_label.npy')#标签

    Y_test = np.load('test_label.npy')

    Z_train = np.load('train_snr.npy')#你用不上

    Z_test = np.load('test_snr.npy')#你用不上
    ...
    model.add(Reshape(in_shap, input_shape=in_shap))

    model.add(ZeroPadding2D((0, 2)))

    model.add(Conv2D(256, (3, 3), strides=1, data_format='channels_last', padding='valid',
    ...
    model.add(Conv2D(128, (3, 3), strides=1, data_format='channels_last', padding='valid',
    ...
    model.add(Dense(len(classes), activation='softmax', init='he_normal'))

    model.add(Reshape([len(classes)]))

    sgd = SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)

    # model.load_weights('model_weights_f_swt_lv2.h5')
    ...
    test_Y_hat = model.predict(X_test, batch_size=150)
    ...
    model.save_weights('model_weights_f_swt1.h5')
